...
import { itemPriceAdjusterVersion2Builder } from '../builders/itemPriceAdjusterVersion2Builder'
...
describe('ItemPriceAdjusterVersion2', () => {
  describe('price is less than 100', () => {
    it('marks item price up by the markup percentage', async () => {
      ...
      const sut = itemPriceAdjusterVersion2Builder().withMarkUpPercentage(20).build()
      // Act
  ...
  describe('price is greater than 100', () => {
    it('marks item price down by the markdown percentage', async () => {
      ...
      const sut = itemPriceAdjusterVersion2Builder().withMarkDownPercentage(20).build()
      // Act
      const result = await sut.adjustPrice(item)
      // Assert
      expect(result.price).toEqual(expected)
  ...
  describe('price is equal to 100', () => {
    it('will not alter the price', async () => {
      ...
      const sut = itemPriceAdjusterVersion2Builder().build()
      // Act
      const result = await sut.adjustPrice(item)
      // Assert
      expect(result.price).toEqual(expected)
